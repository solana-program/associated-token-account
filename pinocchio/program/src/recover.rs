...
pub(crate) fn process_recover_nested(
    ...
    let [nested_ata, nested_token_mint, destination_ata, owner_ata, owner_token_mint, wallet, token_program, ..] =
        accounts
    else {
    ...
    if !owner_token_mint.owned_by(token_program.address()) {
    ...
    if !owner_ata.owned_by(token_program.address()) {
    ...
    if !nested_ata.owned_by(token_program.address()) {
    ...
    if !nested_token_mint.owned_by(token_program.address()) {
